  ...
  server.registerTool(
    ...
    async ({ path }) =>
      callTool("list_dir", async () => {
        if (config.hermesMode === "http") return hermesClient.listDir(path);
  ...
  server.registerTool(
    ...
    async ({ path }) =>
      callTool("read_file", async () => {
        if (config.hermesMode === "http") return hermesClient.readFile(path);
  ...
  server.registerTool(
    ...
    async ({ command, cwd }) =>
      callTool("run_command", async () => {
        ...
        if (config.hermesMode === "http") return hermesClient.runCommand(safeCommand.command, resolvedCwd);
